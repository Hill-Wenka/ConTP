...
class ProteinDataset(Data.Dataset):
    def __init__(self, name, path=None, sequence=None, metadata=None, dir='dataset', **kwargs):
        ...
        self.metadata = pd.DataFrame(
            columns=['name', 'sequence', 'length'])  # include index, name, sequence, label, partition, etc.
        ...
        self.h5_index_map = {i: i for i in range(
            len(self.metadata))}  # map the index of the subset to the original dataset for lazy loading

        # whether to load feature data when getting an item
        self.load_item_feature = kwargs.get('load_item_feature', True)
    ...
    def load_metadata(self, indices=None):
        if os.path.exists(self.csv):
            # print(f'Loading metadata from {self.csv}')
    ...
    def construct_subset(self, indices, name=None):
        ...
        for feature_name, feature_data in self.features.items():
            if isinstance(feature_data, (np.ndarray, torch.Tensor)):
                ...
                subset.h5_index_map = {i: j for i, j in
                                       enumerate(indices)}  # map the index of the subset to the original dataset
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.load_item_feature:  # no feature data
            for feature_name, feature_data in self.features.items():
